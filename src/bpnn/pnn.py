...
"""
Adaptations from https://github.com/DLR-RM/BPNN/
...
class ProgressiveNeuralNetwork(nn.Module):
    ...
    def load_full_state_dict(self, full_state_dict: Dict[str, Any]):
        ...
        if full_state_dict['networks']:
            num_columns = max(int(re.findall(r'\d+', key)[0]) for key in full_state_dict['networks'].keys()) + 1
...
class DropoutProgressiveNeuralNetwork(ProbabilisticProgressiveNeuralNetwork):
    ...
    def __init__(
        ...
        for dropout_position in dropout_positions:
            if lateral_connections is not None:
...
def dataset_step_pnn(
    ...
    """
    if type(loss_function) is nn.CrossEntropyLoss or type(loss_function) is nn.BCEWithLogitsLoss:
