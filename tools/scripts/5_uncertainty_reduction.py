...
# TODO: get saving functions different
# TODO: get the query and probability saver
...
def main(args):
    ...
    # evaluate here the probabilities
    prob_knowns, prob_unknowns = evaluate_uncertainty_reduction(
        args=args, model=clever.classifier, dataloaders=test_loader, device=device)
    knowns.append(list(prob_knowns))
    unknowns.append(list(prob_unknowns))
    print("prob_knowns: ", np.mean(prob_knowns))
    print("prob_unknowns: ", np.mean(prob_unknowns))
    ...
    # saving the results
    results = {}
    results['knowns'] = knowns
    results['unknowns'] = unknowns

    # save the latest one
    filename = args.results_path + "5_uncertainty_reduction_" + args.acquisition + "_" + args.model + "_" + args.object \
