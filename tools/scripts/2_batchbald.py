...
"""
1. Take a common test set from all eight scenarios.
2. Take few samples from each demonstrations.
3. Repeat it for all the objects on Slurm node.
4. Take seeds after seeing good performance.

Expectation: 
BatchBald results in better performance than BALD.
# Try with (1) CLEVER with, without prior and with deterministic.
"""
import argparse
import copy
import tqdm
...
def main(args):
    ...
    if args.model=="TheCleverSystem":
        clever = TheCleverSystem(args=args, class_mapping={args.object: 0}, device=device)
    ...
    while True:
        ...
        for i in range(0, args.num_epoch):
            clever.active_learn(object_name=args.object, dataloader=(train_loader, train_loader, test_loader)) # TODO: different!
        ...
        # NOTE: clever.metric takes from the MAP estimates
        # But evaluation point of view, it is fair to do so since
        # we aim to compare different acquisition functions.
        ...
        if args.subsample: # TODO: how to verify this further?
    ...
    results['num_subsample'] = args.num_subsample
    results['test_loss'] = test_loss
    results['test_accs'] = test_accs
    results['added_indices'] = added_indices

    # save the latest one
    filename = args.results_path + "2_batchbald_" + args.acquisition + "_" + args.model + "_" + args.object \
